...
void salvarAtendidos(const vector<Paciente>& atendidos) {
    ...
    for(int i=0;i<atendidos.size();i++){//loop pra armazenar os dados do paciente no txt
...
void carregarAtendidos(vector<Paciente>& atendidos) {
   ...
   //se o arquivo não existir imprimir
    ...
    while (getline(arquivo, p.Nome)) {//enquanto tiver algo para ser lido ele ira preencher a struct paciente p com os dados abaixo
